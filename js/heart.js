...
const renderer = new THREE.WebGLRenderer({
  antialias: true,
  alpha: true //  是否执行抗锯齿。默认值为false。
});

// 设置颜色及其透明度
// renderer.setClearColor(new THREE.Color("rgba(0,0,0)"));
...
new THREE.OBJLoader().load(
  ...
  (obj) => {
    ...
    heart.material = new THREE.MeshBasicMaterial({
      color: new THREE.Color("rgba(0,0,0,0)"),
    });
...
const palette = [
  new THREE.Color("#fa86c4"),
  new THREE.Color("#f2cdcd"),
  new THREE.Color("#fc46aa"),
...
function render(a) {
  ...
  heart.geometry.attributes.position.needsUpdate = false;
